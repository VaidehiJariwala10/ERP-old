    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        const loadAttendance = async (month, year) => {
            try {
                ...
                if (data.status === 'success') {
                    ...
                    isSundayOff = data.data.isSundayOff || false;
                    isSaturdayOff = data.data.isSaturdayOff || false;
                    populateUserFilter();
                    renderCalendar();
                }
            } catch (error) {
        ...
        const renderMobileDateScroll = () => {
            ...
            for (let day = 1; day <= daysInMonth; day++) {
                ...
                else if (saturdayOffDates.includes(dateStr) || (dayOfWeek === 6 && isSaturdayOff)) dateCard.classList.add('saturday-off');
                else if (dayOfWeek === 0 && isSundayOff) dateCard.classList.add('sunday');
            ...
            `;
                ...
                if (holiday) {
                    ...
                    label.textContent = holiday.title || 'Holiday';
                    dateCard.appendChild(label);
                } else if (saturdayOffDates.includes(dateStr) || (dayOfWeek === 6 && isSaturdayOff)) {
                    const label = document.createElement('div');
                    label.className = 'date-card-label saturday-off';
                    label.textContent = 'Sat Off';
                    dateCard.appendChild(label);
                } else if (dayOfWeek === 0 && isSundayOff) {
                    const label = document.createElement('div');
                    label.className = 'date-card-label sunday';
                    label.textContent = 'Sunday Off';
                    dateCard.appendChild(label);
                }
        ...
        const renderMobileAttendanceList = () => {
            ...
            if (holiday || saturdayOffDates.includes(selectedDate) || (dayOfWeek === 6 && isSaturdayOff) || (dayOfWeek === 0 && isSundayOff) || isFuture){
                mobileList.innerHTML = `
                <div class="alert alert-info text-center">
                    ${holiday ? `<strong>Holiday:</strong> ${holiday.title}` :
                        (saturdayOffDates.includes(selectedDate) || (dayOfWeek === 6 && isSaturdayOff)) ? '<strong>Saturday Off</strong>' :
                            (dayOfWeek === 0 && isSundayOff) ? '<strong>Sunday Off</strong>' :
        ...
        const renderDesktopCalendar = () => {
            ...
            `;
                ...
                if (holiday) {
                    ...
                    sunLabel.textContent = 'Sunday Off';
        ...
        const userFilterEl = $('#user-filter');
        if (userFilterEl.length) {
            userFilterEl.on('change', () => {
                if (isMobile) {
                    renderMobileAttendanceList();
                } else {
                    renderCalendar();
