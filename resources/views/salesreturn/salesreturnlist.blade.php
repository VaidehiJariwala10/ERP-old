...
@section('content')
    <style>
        ...
        /* =============================================
                   ...
                   (identical pattern to Sales List)
                ============================================= */
        .mobile-order-card {
            display: none;
        }
        ...
        /* Word wrapping for customer name in desktop table */
...
/* Word wrapping for customer name in mobile card view */
.mobile-detail-row:has(.mobile-detail-label:contains("Customer")) .mobile-detail-value {
    word-wrap: break-word;
    word-break: break-word;
    white-space: normal !important;
}

/* Alternative: If the above :contains doesn't work, use this for mobile */
.mobile-detail-row .mobile-detail-label {
    font-weight: 600;
    color: #595b5d;
    font-size: 14px;
}
...
/* Specifically target customer row in mobile */
.mobile-detail-row:first-child .mobile-detail-value,
.mobile-detail-row:nth-child(4) .mobile-detail-value {
    word-wrap: break-word;
    word-break: break-word;
    white-space: normal !important;
}
...
/* Only customer column should wrap */
.table-responsive table td:nth-child(5) {
    white-space: normal !important;
}
    </style>
    ...
    <div class="content">
        <div class="page-header">
            ...
            <div class="page-btn">
                ...
                <a href="{{ route('salesreturn.add') }}" class="btn btn-added">
        ...
        $(document).ready(function() {
            ...
            window.showItemsModal = function(returnId) {
                ...
                if (!returnData || !returnData.items || returnData.items.length === 0) {
                    Swal.fire({
                        title: 'No Items',
                        text: 'No items found for this sales return.',
                        icon: 'info',
                ...
                returnData.items.forEach((item, index) => {
                    let gstText = 'N/A';
                    if (Array.isArray(item.gst_details) && item.gst_details.length > 0) {
                        gstText = item.gst_details.map(tax => {
                            return `${tax.tax_name}: ${tax.tax_rate}% (${formatCurrency(tax.tax_amount)})`;
                        }).join('<br>');
                    } else if (item.gst_details === 'inclusive') {
                        gstText = 'Inclusive';
                    }
                    ...
                    itemsHtml += `
                        <tr>
                            ...
                            <td>${formatCurrency(item.discount_amount)} (${item.discount}%)</td>
            ...
            // (same pattern as Sales List toggleMobileDetails)
            // =============================================
            ...
            // (same structure as renderMobileOrders in Sales List)
            // =============================================
            function renderMobileReturnCards(data) {
                ...
                if (!data || data.length === 0) {
                    container.html('<div class="text-center p-4">No sales return records found.</div>');
                    return;
                }
                ...
                data.forEach((item, index) => {
                    ...
                    const orderLink = item.order_number ?
                        `<a href="/sales-details/${item.order_id}" class="mobile-order-ref" style="color:#ff9f43;font-weight:600;">${escapeHtml(item.order_number)}</a>` :
                        '-';
                    const customerName = escapeHtml(item.customer) || '-';

                    const cardHtml = `
                        <div class="mobile-order-item" data-return-id="${item.id}">

                            <!-- Always visible: Order Number + toggle button -->
                            <div class="mobile-order-row">
                                <div class="mobile-order-cell mobile-order-number" style="color:black;">
                                    <span class="mobile-order-party">${customerName}</span>${item.order_number ? `${orderLink}` : '-'}
                                </div>
                            ...
                            <!-- Expandable details (hidden by default, same as Sales List) -->
                            <div class="mobile-order-details" id="mobile-return-details-${item.id}">

                                <div class="mobile-detail-row">
                                    <span class="mobile-detail-label">Return No:</span>
                                    <span class="mobile-detail-value">${escapeHtml(item.return_number) || '-'}</span>
                                </div>
                                <div class="mobile-detail-row">
                                ...
                                <div class="mobile-detail-row">
                                    ...
                                    <span class="mobile-detail-value">${escapeHtml(item.customer) || '-'}</span>
                                </div>
                                <div class="mobile-detail-row">
                                ...
                                <div class="mobile-detail-row">
                                    ...
                                    <span class="mobile-detail-value">${parseFloat(item.return_qty || 0).toFixed(2)}</span>
                                </div>
                                <div class="mobile-detail-row">
                                ...
                                <div class="mobile-detail-row">
                                    ...
                                    <span class="mobile-detail-value">${discountAmount}</span>
                                </div>
                                <div class="mobile-detail-row">
            ...
            // Fetch Sales Returns
            // =============================================
            function fetchSalesReturns(page = 1) {
                let url = `/api/sales-return-list?page=${page}&per_page=${perPage}`;
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        if (!response.status) {
                            renderEmpty(response.message || "No sales return records found.");
                            $('.pagination-controls').hide();
                            return;
                        }
                        ...
                        const rows = allReturnData.map((item, index) => {
                            ...
                            return [
                                ...
                                `<span>${parseFloat(item.return_qty || 0).toFixed(2)}</span>`,
                        ...
                        if (rows.length > 0) {
                            ...
                            table.draw();
                            renderEmpty("No sales return records found.");
                            $('.pagination-controls').hide();
                        }
                    ...
                    error: function(xhr) {
                        console.error("Sales return list API error:", xhr);
                        renderEmpty("Unable to load sales return records.");
                        $('.pagination-controls').hide();
                    }
                });
            }
