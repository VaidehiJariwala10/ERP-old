...
@section('content')
    ...
    <div class="content">
        <div class="page-header">
            <div class="page-title"><h4>Edit Meeting</h4></div>
            ...
            <div class="card-body">
                <form id="meetingForm" method="POST" novalidate>
                    ...
                    <div class="row">
                        ...
                        <div class="col-lg-12">
                            @if ($canEditMeeting)
                                <button type="submit" id="submitMeetingBtn" class="btn btn-submit me-2">Update</button>
                            @endif
                            @if ($canViewMeeting)
...
<script>
    $(document).ready(function () {
            ...
            $.ajax({
                ...
                success: function (response) {
                    if (response.status) {
                        ...
                        if (selectedCustomerId) $('#customer_id').val(String(selectedCustomerId)).trigger('change');
                    }
                }
            });
        }
        ...
        function loadStaff(selectedStaffId) {
            if (isStaffUser) {
            ...
            $.ajax({
                ...
                success: function (response) {
                    if (response.status) {
                        ...
                        $('#assigned_to').html(options);
                        if (selectedStaffId) {
                            $('#assigned_to').val(String(selectedStaffId)).trigger('change');
        ...
        // Load meeting data and populate form
            ...
            $.ajax({
                ...
                success: function (response) {
                    if (response.status) {
                        ...
                        // Load dropdowns with pre-selected values
                        loadCustomers(m.customer_id);
                        loadStaff(isStaffUser ? loggedInStaffId : m.assigned_to);

                        // Populate plain fields immediately
                        $('#meeting_title').val(m.meeting_title || '');
                        ...
                        if (m.scheduled_on) {
                            const date = new Date(m.scheduled_on);
                            const localDateTime = new Date(date.getTime() - date.getTimezoneOffset() * 60000).toISOString().slice(0, 16);
                            $('#scheduled_on').val(localDateTime);
                        }
                    } else {
                        Swal.fire({ title: 'Error!', text: response.message || 'Meeting not found.', icon: 'error', confirmButtonText: 'OK' })
                            .then(() => { window.location.href = "{{ route('meeting.list') }}"; });
                    }
                },
                error: function () {
                    Swal.fire({ title: 'Error!', text: 'Failed to load meeting data.', icon: 'error', confirmButtonText: 'OK' })
                        .then(() => { window.location.href = "{{ route('meeting.list') }}"; });
                }
            });
        }
        ...
        $('#meetingForm').submit(function (e) {
            ...
            $.ajax({
                url: `/meeting/${meetingId}/update`, type: 'POST',
                ...
                success: function (response) {
                    ...
                    if (response.status) {
                        Swal.fire({ title: 'Success!', text: 'Meeting updated successfully!', icon: 'success', confirmButtonText: 'OK', confirmButtonColor: '#ff9f43' })
                            .then(() => { window.location.href = "{{ route('meeting.list') }}"; });
                    } else {
                        Swal.fire({ title: 'Error!', text: response.message || 'Failed to update meeting.', icon: 'error', confirmButtonText: 'OK' });
                    }
                },
                error: function (xhr) {
                    $btn.html(originalText).prop('disabled', false);
